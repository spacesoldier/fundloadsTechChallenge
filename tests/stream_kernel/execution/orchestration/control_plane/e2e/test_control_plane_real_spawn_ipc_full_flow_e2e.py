...
def test_real_spawn_ipc_full_flow_startup_boundary_handoff_and_runtime_shutdown() -> None:
    ...
    state.append_event(
        ControlPlaneLaunchPlanEvent(
            plan=ControlPlaneLaunchPlan(
                groups=(ControlPlaneGroupSpec(group_name="execution.alpha", workers=1, nodes=("remote.node",)),)
            )
        )
    )
    ...
    root_shutdown = DefaultControlPlaneRootShutdownService(
        root_stop=root_stop,
        worker_lifecycle=lifecycle,
    )
    group_router = InMemoryProcessGroupRouterService()
    group_router.configure_process_groups([{"name": "execution.alpha", "nodes": ["remote.node"]}])
    ...
    root_runtime_lifecycle = ControlPlaneRootRuntimeLifecycleManager(
        state=state,
        root_shutdown=root_shutdown,
        fallback_terminate_timeout_seconds=0.5,
    )

    handle = lifecycle.spawn_worker(
        target_id="execution.alpha#1",
        target=leaf_full_flow_worker,
        args=(),
        name="sk:test-full-flow",
        daemon=True,
        start=True,
    ...
    try:
        ...
        terminal = root_handoff.drain_external_deliveries(
            envelopes=[Envelope(payload={"payload": 1}, target="remote.node", trace_id="t-1")],
            source_group="execution.root",
        )
        deadline = time.monotonic() + 2.0
        while time.monotonic() < deadline and not terminal:
            terminal = root_handoff.drain_completed_deliveries()
            if terminal:
                break
            time.sleep(0.01)
        assert terminal in ([], [{"ok": True, "worker_id": "execution.alpha#1", "count": 1}])

        root_runtime_lifecycle.start()
        assert root_runtime_lifecycle.ready(1) is True
        ...
        assert any(isinstance(e, ControlPlaneLeafBoundaryResultEvent) for e in events)
        assert any(isinstance(e, ControlPlaneLeafStopAckEvent) for e in events)
    finally:
        lifecycle.stop_worker(handle.target_id, graceful_timeout_seconds=0.05, terminate_timeout_seconds=0.2)
        adapter.close()
