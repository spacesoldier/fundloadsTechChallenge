...
def test_real_spawn_ipc_stop_roundtrip_root_to_leaf_and_back() -> None:
    ...
    handle = lifecycle.spawn_worker(
        ...
        target=leaf_stop_worker,
        args=(),
        name="sk:test-stop-roundtrip",
        daemon=True,
        start=True,
    ...
    try:
        ...
        assert stop_ack.status == "accepted"
        assert stop_ack.command_id == "stop-1"
        assert any(
            isinstance(event, ControlPlaneLeafStopAckEvent) and event.command_id == "stop-1"
