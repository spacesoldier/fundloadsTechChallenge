...
def test_p5pre_sup_10_worker_loop_can_write_jsonl_lifecycle_logs(tmp_path) -> None:
    # BOOT-LOG-04: child worker lifecycle events should be written to per-worker file sink.
    from stream_kernel.platform.services.runtime import bootstrap as bootstrap_module
    ...
    bootstrap_module._worker_loop(  # noqa: SLF001 - contract probe
        ...
        "execution.cpu",
        {
            "exporters": [{"kind": "jsonl", "settings": {"path": str(tmp_path / "supervisor.jsonl"), "workers_dir": str(workers_dir)}}],
            "lifecycle_events": {"enabled": True, "level": "debug"},
        },
    )

    worker_log_path = workers_dir / "execution.cpu_1.jsonl"
    assert worker_log_path.exists()
    rows = [json.loads(line) for line in worker_log_path.read_text(encoding="utf-8").splitlines() if line.strip()]
